...
- constituent/weight reconstruction source: **TOPIX month-end index master (TOPIX月末指数マスタ)**
- earliest month-end master in the published DataCube specification: **2015-11-30**
...
2. The specification says the master contains constituent-level information as of the last business day of each month and is available from the 2015-11 month-end onward.
3. The master includes local security code, name, ISIN, index code/classification, index-calculation close, number of shares, FFW and closing market value (`CMV`). Those fields provide the point-in-time inputs needed to reconstruct and reconcile free-float market-cap benchmark weights deterministically in #50; the decision does not assume an undocumented ready-made `weight` column.
...
The **earliest point-in-time constituent snapshot under this adopted route is the 2015-11-30 month-end master**. That does not mean a portfolio decision may be dated 2015-11-30: the snapshot must first become available under the publication rule below. A first headline evaluation using this route must begin only at a legitimately tradable decision/execution point after that first snapshot became available, unless a later issue adopts a separately licensed earlier point-in-time source before results are inspected.
...
- reconstruct benchmark weights from the documented point-in-time fields and reconcile the resulting weights/counts before use;
...
The exact evaluation period and acquired source snapshots remain later bounded decisions, but they must be fixed before outcome inspection. Under this selected month-end-master route, the first usable constituent snapshot is effective 2015-11-30 and may influence a decision only after its documented next-month publication time and the applicable decision lag.
...
- Nikkei 225 profile / price-weighted method: https://indexes.nikkei.co.jp/en/nkave/index/profile
...
First point-in-time constituent snapshot under adopted route: **TOPIX month-end index master effective 2015-11-30; usable only after documented publication availability and lag**  
