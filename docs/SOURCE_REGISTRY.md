...
| `blackrock-ishares-1475-holdings` | iShares Core TOPIX ETF (1475) dated holdings | BlackRock Japan | Dated ETF holdings used only as an investable control allocation for Historical Replay | Official web/CSV; low-frequency manual retrieval of pinned dates | TSE ticker/security code | High for the ETF's own disclosed holdings, not evidence of issuer conduct | `LOCAL_RESEARCH_ALLOWED` for private-use storage/display/analysis/format changes under BlackRock Japan site terms checked 2026-09-10; redistribution/publication remains restricted, so raw rows stay local-only | **WATCH** | Not official TOPIX constituent weights; `as_of_date` does not establish historical `available_at`, so unverified cutoff availability must block |
